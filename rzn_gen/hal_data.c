...
static const xspi_qspi_extended_cfg_t g_qspi0_extended_cfg = { .unit = 0,
		.chip_select = XSPI_QSPI_CHIP_SELECT_0, .memory_size =
				XSPI_QSPI_MEMORY_SIZE_64MB, .p_timing_settings =
				&g_qspi0_timing_settings, .prefetch_en =
				XSPI_QSPI_PREFETCH_FUNCTION_DISABLE, };
const spi_flash_cfg_t g_qspi0_cfg = { .spi_protocol =
		SPI_FLASH_PROTOCOL_1S_1S_1S, .address_bytes = SPI_FLASH_ADDRESS_BYTES_3,
		.dummy_clocks = SPI_FLASH_DUMMY_CLOCKS_6, .read_command = 0xEB,
		.page_program_command = 0x02, .write_enable_command = 0x06,
		.status_command = 0x05, .write_status_bit = 0,
		.xip_enter_command = 0x20, .xip_exit_command = 0xFF,
...
const spi_flash_instance_t g_qspi0 = { .p_ctrl = &g_qspi0_ctrl, .p_cfg =
		&g_qspi0_cfg, .p_api = &g_spi_flash_on_xspi_qspi, };
