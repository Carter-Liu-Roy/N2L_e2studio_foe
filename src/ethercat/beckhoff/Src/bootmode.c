...
void BL_StartDownload(UINT32 password)
{
	...
	EraseSectorNumber = 0;
	EraseSectorFlag = 0;
	MEMCPY(&FlashWriteBuffer[0], (UINT8 *)(FW_UP_BANK0_ADDR - FW_UP_MIRROR_OFFSET), FW_UP_PAGE_SIZE);
...
UINT16 BL_Data(UINT16 *pData,UINT16 Size)
{
	UINT16 ErrorCode = 0;
	UINT8  LastData;
	volatile UINT32 *pIdentify = (UINT32 *)(FW_UP_BANK1_ADDR + FW_UP_APPLI_ID_OFFSET - FW_UP_MIRROR_OFFSET);
	UINT32 i;
	UINT32 ldr_addr;
	...
	case BL_DATA_STATUS_WRITE:
		...
		{
			MEMCPY(&FlashWriteBuffer[0], (UINT8 *)(FW_UP_BANK1_ADDR - FW_UP_MIRROR_OFFSET), FW_UP_PAGE_SIZE);
	...
	case BL_DATA_STATUS_WRITE_LDRPRM:
		...
		if(true != status_erase.write_in_progress)							// Is flash erasing end ?
		{
			...
			{
				DataStatus = BL_DATA_STATUS_ERASE_START;					// yes. next -> erase BANK1
			}
			else
			{
				DataStatus = BL_DATA_STATUS_SII_UPDATE;						// No. write BANK1 LDRPRM next -> update SII.
			}
			ErrorCode = FOE_MAXBUSY_ZERO;									// return "BUSY"
		}
		else
	...
	case BL_DATA_STATUS_SII_UPDATE:
		//--------------------------------------------------
		// SII uodate, update firmware Revision Number
		//--------------------------------------------------
		...
		fw_up_close();
		BL_SetRebootFlag(TRUE);												// yes. reboot is avalilable.
		break;
...
void BL_EraseSector(uint16_t offset_sector_number)
{
	...
	EraseSectorNumber = offset_sector_number;
...
static void BL_SectorIsErased(uint16_t offset_sector_number)
{
	EraseSectorFlag |= (1U << offset_sector_number);
} /* BL_SectorIsErased */
