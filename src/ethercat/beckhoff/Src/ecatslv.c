...
void AL_ControlInd(UINT8 alControl, UINT16 alStatusCode)
{
    ...
    UINT8            bErrAck = 0;
    UINT8         stateTrans;
    INT16         retval;	// REL_SII_160824
	EEPBUFFER     Buffer;	// REL_SII_160824
    ...
    if ( result == 0 )
    {
        ...
        case INIT_2_BOOT    :
            ...
            BL_SetRebootFlag(FALSE);	// REL_FoE_140918
...
/*ECATCHANGE_END(V5.13) ECAT1*/
        ...
        case INIT_2_PREOP :
           ...
           UpdateEEPROMLoadedState();
			...
			/* SII firmware version verification , check SII and firmware Revision No */ 		// REL_SII_160824
			retval = ESC_EepromAccess(SII_EEP_IDENTIFY_OFFSET, 8, &Buffer.word[0], ESC_RD);		// REL_SII_160824
			if(Buffer.dword[SII_EEP_REVESIONNO/2] != tbl_identify[ID_REVISION])					// REL_SII_160824
			{																					// REL_SII_160824
#if (FW_UP_ENABLE_REVNO_CHECK == 1)
				result = ALSTATUSCODE_FW_SII_NOT_MATCH;											// REL_SII_160824
#endif
			}
            break;
...
/*ECATCHANGE_START(V5.13) ESM2*/
    ...
    else if ( alControl != (nAlStatus & STATE_MASK) )
    {
		...
		// after correct file download state change BOOT -> INIT 
		if(stateTrans == BOOT_2_INIT && BL_CheckRebootFlag() == TRUE)	// REL_FoE_140918
		{																// REL_FoE_140918
			BL_Reboot();												// REL_FoE_140918
		}																// REL_FoE_140918
